...
import RequestGenericError from '../molecules/RequestGenericError'
import RequestSuccess from '../molecules/RequestSuccess'
...
export default function EditPetForm({ animal }: Props) {
  ...
  async function handleEditPet(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!petId) {
      setFeedback({
        type: 'error',
        message: 'Animal não encontrado para edição.',
        key: Date.now(),
      })
      return
    }
  ...
  return (
    <>
      ...
      {feedback?.type === 'error' && (
        <RequestGenericError key={feedback.key} message={feedback.message} />
      )}
